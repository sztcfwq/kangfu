...
begin
  ...
  begin
    ...
    qf.Open;
    if qf.RecordCount >0 then continue;
    qf.Close;
    qf.sql.Clear;
...
procedure Tfrm_main.Image5Click(Sender: TObject);
begin
//显示其他界面
  nb.ActivePage :='qt';
  tishi.Caption :='';
...
begin
  ...
  //从csey.this4中根据住院号或门诊号找到病人信息
    ...
    begin
      qf.Close;
      qf.sql.clear;
      qf.sql.add('select dz,dw from kf_brxxk where xm=:axm and xb=:axb and nl=:anl');
...
var
  zyh,bh,xm,xb,nl,dz,dw,zdzz,yp,zdr,czy,zlpcs,mzlx:string;
...
//保存记录到数据库中,然后根据条件生成治疗单据
  ...
  if ((xm='')or(xb='')or(bh='')or(cfjls<=1)) then
  begin
  ...
  qf.Close;
  qf.sql.clear;
  qf.sql.add('select bh,zdid from kf_brzdk where bh=:abh');
  qf.Parameters.ParamByName('abh').Value :=bh;
  qf.Open;
  if qf.RecordCount >0 then
  begin
    ...
    qf.Close;
    qf.sql.Clear;
    qf.sql.add('update kf_brzdk set zdzz=:azz,cf=:acf,sfje=:aje where bh=:abh');
    ...
    messagebox(self.Handle,'保存病人处方信息成功！','建卡',MB_ICONINFORMATION);
    exit;
  end;
  qf.Close;
  qf.SQL.clear;
  ...
  //生成治疗流水记录
  sczlmx(zdid);
  messagebox(self.Handle,'保存病人信息成功！','建卡',MB_ICONINFORMATION);
end;
...
//打印病人治疗单
  ...
  q_cf.Close;
  q_cf.SQL.clear;
  q_cf.SQL.Add('select a.jlzt, left(a.xmmc,10) as xmmc,a.xmsl from kf_brcfmxk a,kf_brzdk b where a.zdid=b.zdid and b.bh=:abh');
  q_cf.Parameters.ParamByName('abh').Value :=bh;
  q_cf.Open;
  ...
  'b.sfje from kf_brxxk a join kf_brzdk b on a.brid=b.brid where b.bh=:abh');
  qp.Parameters.parambyname('abh').Value :=bh;
  qp.Prepared :=true;
  qp.Open;
  qf.Close;
  qf.SQL.clear;
  ...
  ' where b.bh=:abh and a.wcbz=0 order by a.lsid');
  qf.Parameters.ParamByName('abh').Value :=bh;
  qf.Prepared :=true;
  qf.Open;
  dlry:=trim(frm_dl.edt_zgdm.Text);
